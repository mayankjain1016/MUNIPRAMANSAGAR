...
export default function SantKiAdalatManagement() {
  ...
  const fetchVideos = async () => {
    try {
      const response = await fetch(`${API_URL}/sant-ki-adalat/admin`, {
        credentials: 'include',
  ...
  const handleSubmit = async () => {
    try {
      const response = editMode
        ? await fetch(`${API_URL}/sant-ki-adalat/${currentId}`, {
            method: 'PUT',
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(formData)
          })
        : await fetch(`${API_URL}/sant-ki-adalat`, {
            method: 'POST',
            credentials: 'include',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify(formData)
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this video?')) {
      try {
        const response = await fetch(`${API_URL}/sant-ki-adalat/${id}`, {
          method: 'DELETE',
          credentials: 'include',
          headers: { 'Content-Type': 'application/json' }
        });
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 4, flexWrap: 'wrap', gap: 2 }}>
        <Box>
          <Typography variant="h4" sx={{ fontWeight: 700, color: '#0f172a', mb: 0.5, fontSize: { xs: '1.5rem', sm: '1.875rem' } }}>
            संत की अदालत Management
          </Typography>
          <Typography variant="body2" sx={{ color: '#64748b', fontSize: '0.9rem' }}>
            Manage Sant Ki Adalat videos and content
          </Typography>
        </Box>
        <Button 
          variant="contained" 
